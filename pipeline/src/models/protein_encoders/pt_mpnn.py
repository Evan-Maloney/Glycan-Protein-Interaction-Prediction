...
class MPNNProteinEncoder(ProteinEncoder):
    def __init__(self, embedding_dim: int = 128, pos_emb_dim: int = 2,
                 hidden_state_size: int = 128, n_layers: int = 3):
        super().__init__()
        # Use the glycan MPNN to handle SMILES conversion.
        self.mpnnglycan = MPNNGlycanEncoder(embedding_dim=embedding_dim,
    ...
    def encode_sequence(self, sequence: str, device: torch.device) -> torch.Tensor:
        """
        Converts a protein sequence to a molecule using RDKit's Chem.MolFromSequence,
        then obtains a SMILES string and feeds it into the MPNN.
        """
        mol = Chem.MolFromSequence(sequence)
        if mol is None:
            raise ValueError(f"Could not convert protein sequence to molecule: {sequence}")
        smiles = Chem.MolToSmiles(mol)
        return self.mpnnglycan.encode_smiles(smiles, device)
    
    def encode_batch(self, batch_data: List[str], device: torch.device) -> torch.Tensor:
        embeddings = [self.encode_sequence(seq, device) for seq in batch_data]
        return torch.cat(embeddings, dim=0).to(device)
    
    @property
    def embedding_dim(self) -> int:
        return self.mpnnglycan.embedding_dim
    
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Optionally define a forward pass if needed.
        return x
