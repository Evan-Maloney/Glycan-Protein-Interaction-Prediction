...
class SweetTalkGlycanEncoder(GlycanEncoder):
    def __init__(self, model_path: str = './data/469_glycan_SweetTalk_large2.pt', embedding_dim: int = 29, device: str = 'cpu'):
        super().__init__()
        ...
        self.num_classes = 29 #len(self.lib_all_long)+1
    ...
    def encode_iupac(self, iupac_str: str, device: torch.device) -> torch.Tensor:
        ...
        print('embedded shape right before bn1', embedded.shape)

        #embedded = self.model.bn1(embedded)  # This is where the error occurs
    ...
    def encode_batch(self, batch_data: List[str], device: torch.device) -> torch.Tensor:
        ...
        print('iupac batch:', iupac_encoding_batch.shape)
