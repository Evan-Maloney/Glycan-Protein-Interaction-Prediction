# Used Maxym's GNN.py to implement GAT
...
class GATGlycanEncoder(GlycanEncoder):
    def __init__(self, embedding_dim: int = 256, hidden_channels: int = 128):
        ...
        # Define GAT layers
        self.conv1 = GATConv(9, hidden_channels//2, heads=4, concat=True)  # 9 is the expanded node features
        self.conv2 = GATConv((hidden_channels//2)*4, hidden_channels, heads=4, concat=True)
        self.conv3 = GATConv(hidden_channels*4, hidden_channels*2, heads=4, concat=True)
        ...
        self.lin3 = torch.nn.Linear(64, embedding_dim)
    ...
    def forward(self, data):
        """Process a batch of molecular graphs through the GAT"""
